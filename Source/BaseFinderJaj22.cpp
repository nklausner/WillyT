...
namespace BaseFinderJaj22
{
	...
	void Init()
	{
		...
		for (int off : potbase)
		{
			...
			wilexpo::all.push_back({});
			Expo &expo = wilexpo::all.back();
			expo.tile = TilePosition((off - mapx - 3) % scanw, (off - mapx - 3) / scanw);
			expo.posi = Position(expo.tile.x * 32 + 64, expo.tile.y * 32 + 48);
			expo.tpos_min = Position(expo.tile.x * 32 + 48, expo.tile.y * 32 + 36);
			expo.tpos_gas = Position(expo.tile.x * 32 + 48, expo.tile.y * 32 + 48);

			Position &bp = expo.posi;

			if (!Broodwar->getRegionAt(bp)->isAccessible()) {
				expo.is_island = true;
			}
			...
			for (size_t i = 0; i < res.size(); ) {
				...
				if (res[i]->getInitialType().isMineralField()) {
					expo.minerals.push_back(res[i]);
					expo.min_tiles.push_back(res[i]->getTilePosition());
					MarkBorderValue(res[i]->getInitialTilePosition(), 2, 1, -1);
				}
				else {
					expo.geysers.push_back(res[i]);
					expo.gas_tiles.push_back(res[i]->getTilePosition());
